...
class InterplaneterUIApp(MDApp):
    ...
    def build(self):
        ...
        pumps = ["","","Water Pump 3","Chemical Pump","Water Pump 2","Water Pump 1"]
        for i in range(6):
            if i<2:
                continue
            self.pump_button = MDRectangleFlatButton(line_color="cyan",text_color="white",text=pumps[i], pos_hint={'center_x': .12+(i-2)*0.09, 'center_y': .260}, size_hint=(0.08,0.08),font_size = 20)
        ...
        rails = ["Side Rail ","Top Rail "]
        for i in range(2):
            for j in range(2):
                self.motor_button = MDRectangleFlatButton(line_color="cyan",text_color="white",text=rails[i]+direction[j], pos_hint={'center_x': .1+j*0.1+i*0.2125, 'center_y': .160}, size_hint=(0.09,0.08),font_size = 20)
